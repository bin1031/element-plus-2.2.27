...
describe('Table.vue', () => {
  ...
  // test issues #12887
  it('First call getSelectionRows', () => {
    beforeEach(() => {
      const wrapper = mount({
        components: {
          ElTable,
        ...
        methods: {
          ...
          getAllDateWhenNoRowSelected() {
            const multipleTableRef = this.$refs.multipleTableRef
            let val = multipleTableRef.getSelectionRows()
            if (val.length === 0) {
              multipleTableRef.toggleAllSelection()
              val = multipleTableRef.getSelectionRows()
              this.multipleSelection = val
            }
          },
        },
        mounted() {
          this.getAllDateWhenNoRowSelected()
        },
      })
